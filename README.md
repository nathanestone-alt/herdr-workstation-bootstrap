...
- `scripts/ubuntu/receipt-authority.sh` — installs and reconciles the root-owned #961 authority envelope at `/etc/stmodel/issue-961/receipt-authority.json` and its separate receipt body. The tools phase records RTK at the regular canonical `$HOME/.cargo/bin/rtk` path and Python 3.13 at the regular `$HOME/.local/bin/python3.13` path; both are checked for duplicate, stale, symlinked, and tampered identities.
