...
- `scripts/ubuntu/bootstrap.sh` — the committed Ubuntu bootstrap entrypoint; it is run through the external trust anchor documented in [docs/bootstrap-trust-anchor.md](docs/bootstrap-trust-anchor.md).
- `scripts/ubuntu/install-trusted-launcher.sh` — one-time administrative provisioning of the launcher and pinned policy; it is never run as part of ordinary bootstrap execution.
- `scripts/ubuntu/receipt-authority.sh` — installs and reconciles the root-owned #961 authority envelope at `/etc/stmodel/issue-961/receipt-authority.json` and its separate receipt body. The tools phase records the pinned official RTK release (version, URL, archive SHA-256) at the regular canonical `$HOME/.cargo/bin/rtk` path while preserving the managed alias, profile, and hook integration. Python 3.13 at the regular `$HOME/.local/bin/python3.13` path is attested with its controlling `pyvenv.cfg`, effective `base_prefix`/stdlib, and deterministic managed-runtime manifests; all identities are checked for duplicate, stale, symlinked, writable, and tampered state.
...
The Ubuntu trust-boundary entrypoints must be launched through the installed
external launcher. Do not use a mutable checkout as an authority and do not
invoke the committed scripts directly; see [docs/bootstrap-trust-anchor.md](docs/bootstrap-trust-anchor.md).
