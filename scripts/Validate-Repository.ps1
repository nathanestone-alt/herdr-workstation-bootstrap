...
$contentAssertions = @(
    ...
    @{ Path = 'tests\Test-HerdrExcelJobRunner.ps1'; Required = @('Review-job bridge ACL assertion ran before host-owned protection.', 'Review-job host-owned protection was not invoked.', 'HostOwnedTreeProtector', 'Protect-HostOwnedTree -TargetPath $reviewJobPath'); Forbidden = @() },
