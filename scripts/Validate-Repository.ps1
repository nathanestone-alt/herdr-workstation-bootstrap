...
$contentAssertions = @(
    ...
    @{ Path = 'tests\Test-HerdrReviewStaging.ps1'; Required = @('Fully hydrated Cloud Files attributes must not block ReparsePoint.', 'Cloud Files ReparsePoint must remain blocked without an allowed boundary.', 'Cloud Files ReparsePoint must remain blocked outside the allowed boundary.', 'Unrecognized reparse tags must remain blocked by the hydration filter.', 'Snapshot native reads do not propagate the Cloud Files boundary.', 'Copy native reads do not propagate the source Cloud Files boundary.', 'Runner JSON native reads do not preserve explicit Cloud Files boundary plumbing.'); Forbidden = @() },
    @{ Path = 'tests\Test-HerdrWindowsPathPolicy.ps1'; Required = @('Cloud Files file acceptance beneath its configured boundary failed.', 'DirectoryExpected', 'FileExpected'); Forbidden = @() },
