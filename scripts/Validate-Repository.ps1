...
$contentAssertions = @(
    ...
    @{ Path = 'scripts\ubuntu\bootstrap.sh'; Required = @('#!/usr/bin/bash', 'config/ubuntu-toolchain.lock', 'UV_VERSION', 'attestation_create_git_snapshot', 'bootstrap_private_helper', 'bootstrap_trust_fail', 'trusted_git_command', 'attestation_canonical_cargo', 'before-cargo-exec', 'install_receipt_from_snapshots', 'install_python_toolchain', 'write_py_compat', 'download_verified', '/usr/bin/curl', '/usr/bin/sha256sum', 'converge_profile_hook', 'HERDR_PROFILE_CHAIN_ACTIVE', '$HOME/.bash_login', '$HOME/.cargo/bin/$executable', 'cargo_install_root', '--prefix "$node_anchor"', '$node_anchor/lib/node_modules/$package_dir', '@openai/codex@$CODEX_VERSION', '@anthropic-ai/claude-code@$CLAUDE_VERSION', 'toolchain-manifest.txt', 'py_3.13_probe'); Forbidden = @('curl -fsSL https://chatgpt.com/codex/install.sh | sh', 'curl -fsSL https://claude.ai/install.sh | bash', 'fnm install 24', 'rustup default stable') },
    @{ Path = 'scripts\ubuntu\receipt-authority.sh'; Required = @('#!/usr/bin/bash', 'receipt_materialize_helper_from_git', 'receipt_private_helper', 'receipt_trust_fail', 'receipt_canonical_git_storage_path', 'receipt_cleanup'); Forbidden = @() },
    @{ Path = 'scripts\ubuntu\source-attestation.sh'; Required = @('#!/usr/bin/bash', 'attestation_register_temporary_path', 'attestation_cleanup_temporary_paths', 'attestation_snapshot_abort', 'attestation_canonical_cargo'); Forbidden = @() },
    ...
    @{ Path = 'scripts\ubuntu\verify.sh'; Required = @('PATH=/usr/bin:/bin HOME="$HOME" "$login_shell" -lc', 'validate_toolchain_lock', 'verify_resolve_command', 'export PATH="$verify_trusted_path"', 'python3.13', '--list', 'toolchain-manifest.txt', 'PASS login command'); Forbidden = @('HERDR_VERIFY_TEST_MODE', 'HERDR_TEST_LOGIN_PROFILE', 'export PATH="$HOME/.local/bin:$HOME/.cargo/bin') },
